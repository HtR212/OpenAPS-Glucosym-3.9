...
patient_list = [69,71,72,74,77,78,79,80]
...
for filename in os.listdir(directory):
  ...
  ############### End of intermittend fault injection section ##########################
    ...
    data_to_prepend["date"] = int(time.time())*1000+(num_data)*5*60*1000 ##added (_)*5*60*1000
    ...
    current_timestamp = datetime.datetime.fromtimestamp(time.time()+0*60*60+(num_data)*5*60).strftime('%Y-%m-%dT%H:%M:%S') ## Local time
