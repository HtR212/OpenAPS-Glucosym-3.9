...
from .errors import StickError, AckError, BadDeviceCommError # Changed from "from errors import StickError, AckError, BadDeviceCommError"
...
class ProductInfo(StickCommand):
  ...
  rf_table  = { 1: '868.35Mhz' ,	# Changed from "001: '868.35Mhz'"
                0: '916.5Mhz'  ,	# Changed from "000: '916.5Mhz'"
                255: '916.5Mhz'  }
  iface_key = { 3: 'USB',
                1: 'Paradigm RF' }
  ...
  def decode( klass,  data ):
    ...
    , 'description'      : str( data[ 6:16 ] ) # Changed from "data[ 06:16 ]"
  ...
  * download - loop
  ...
  def download_packet(self, size):
    ...
    try:
      ...
      log.info(msg)
      return info
    except BadDeviceCommError as e: # Changed from "except BadDeviceCommError, e:"
  ...
  def clear_buffer(self):
    ...
    for attempt in xrange( 3 ):
      ...
      while size > 14:
        ...
        except BadCRC as e: # Changed from "except BadCRC, e:"
          seg_stats = ( len(segments), sum(map(len, segments)), len(raw) )
          log_detail = segs_vs_raw.format(*seg_stats)
  ...
  def open(self):
    ...
    for attempt in xrange( 1 ):
      try:
        ...
        return True
      except AckError as e: # Changed from "except AckError, e:"
        log.info('failed:(%s):\n%s' % (attempt, e))
        raise
...
if __name__ == '__main__':
  ...
  if not port:
    print("usage:\n%s /dev/ttyUSB0" % sys.argv[0]) # Parenthesis added
    sys.exit(1)
  import link
