...
#initial_glucose = [80]
cmd_main = 'python '+'updated_ct_script_iob_based (copy).py 48'#change "updated_ct_script_iob_based_mitigation_CAWT" to this name before run it
...
for i in browser.find_elements_by_xpath("//*[@type='radio']"):
	...
	if patient_id == 1:#0 or patient_id == 7:#9
		...
		for i in range(1,184):
			day += 1
			df = pd.read_csv("DCLP3_night_new_formated/patient{}/data_patient{}_day{}.csv".format(p,p,i))

			if len(df["CGM_glucose"]) >= 149:
				ig = int(df["CGM_glucose"][84])
			else:
				continue
			...
			csv_file_name = 'data_patient{}_day{}_init{}.csv'.format(p,day,ig)
