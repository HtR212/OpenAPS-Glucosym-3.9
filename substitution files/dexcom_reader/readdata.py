from . import crc16 # Changed from "import crc16"
from . import constants # Changed from "import constants"
from . import database_records # Changed from "import database_records"
import datetime
import serial
import sys
import time
from . import packetwriter # Changed from "import packetwriter"
import struct
import re
from . import util # Changed from "import util"
    ...
    if not device:
      ...
      print ('Found %s S/N: %s'
             % (dex.GetFirmwareHeader().get('ProductName'),
                dex.ReadManufacturingData().get('SerialNumber'))) # Parenthesis added
      print ('Transmitter paired: %s' % dex.ReadTransmitterId())
      print ('Battery Status: %s (%d%%)' % (dex.ReadBatteryState(),
                                           dex.ReadBatteryLevel())) # Parenthesis added
      print ('Record count:') # Parenthesis added
      print ('- Meter records: %d' % (len(dex.ReadRecords('METER_DATA')))) # Parenthesis added
      print ('- CGM records: %d' % (len(dex.ReadRecords('EGV_DATA')))) # Parenthesis added
      print ('- CGM commitable records: %d'
             % (len([not x.display_only for x in dex.ReadRecords('EGV_DATA')])))
      print ('- Event records: %d' % (len(dex.ReadRecords('USER_EVENT_DATA')))) # Parenthesis added
      print ('- Insertion records: %d' % (len(dex.ReadRecords('INSERTION_TIME')))) # Parenthesis added
